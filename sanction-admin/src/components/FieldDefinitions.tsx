...
const INITIAL_DATA: TableSection[] = [
  ...
  },
  {
    ...
    rows: [
      { id: '1', code: 'EXTERNAL_TRADE', text: '외부거래', presets: '외부 거래(EXTERNAL_DEAL), 외부 채널 홍보(EXTERNAL_CHANNEL_PROMO)' },
      { id: '2', code: 'COMMUNITY_VIOLATION', text: '커뮤니티 위반', presets: '문제성 채팅(INAPPROPRIATE_CHAT), 서비스·타인 비방(HARASSMENT), 담합·친목(GROUP_ABUSE)' },
      { id: '3', code: 'PRODUCT_SELLING', text: '판매 위반', presets: '문제성 판매(PROBLEMATIC_SALE), 거래 금지 품목(PROHIBITED_ITEM), 허위·부정 거래(FRAUDULENT_DEAL)' },
      { id: '4', code: 'LIVE_VIOLATION', text: '라이브 위반', presets: '문제성 라이브(INAPPROPRIATE_LIVE), 비효율 라이브(INEFFICIENT_LIVE)' },
      { id: '5', code: 'DEAL_ISSUE', text: '거래 이슈', presets: '배송 문제(DELIVERY_ISSUE), 거래 파기(DEAL_CANCELLATION)' },
      { id: '6', code: 'ACCOUNT_ABUSE', text: '계정 어뷰징', presets: '계정 어뷰징(ACCOUNT_MISUSE), 닉네임/이미지(INAPPROPRIATE_PROFILE)' },
